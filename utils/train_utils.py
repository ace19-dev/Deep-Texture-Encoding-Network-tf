...
def _gather_loss(regularization_losses, scope):
    ...
    # Add the summaries out
    if loss is not None:
        tf.summary.scalar('/'.join(filter(None, ['Losses', scope, 'loss'])),
                          loss)
...
def optimize(optimizer, regularization_losses=None, scope=None, **kwargs):
    """Compute losses and gradients
